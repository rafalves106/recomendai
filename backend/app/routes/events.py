...
def create_event(
    ...
    event = EventModel(
        ...
        if payload.metadata is not None else None,
    )
    db.add(event)
    db.commit()
    db.refresh(event)
...
def get_live_events(db: Session = Depends(get_db)) -> list[LiveEventResponse]:
    # ✅ Ordena por rowid (ordem de inserção no SQLite) em vez de timestamp
    # Isso garante que eventos recém-inseridos apareçam no topo,
    # independentemente do timestamp do evento
    statement = (select(EventModel, ProductModel.name).join(
        ProductModel, ProductModel.id == EventModel.product_id).order_by(
            text("events.rowid DESC")).limit(20))
...
def get_event_stats(db: Session = Depends(get_db)) -> dict[str, int]:
    statement = (select(EventModel.event_type, func.count(
        EventModel.id)).group_by(EventModel.event_type))
    rows = db.execute(statement).all()
...
def get_user_events(
    ...
    statement = (select(EventModel).where(
        EventModel.user_id == user_id).order_by(
            EventModel.timestamp.desc()).limit(limit))
    ...
    for event in events:
        response.append(
            EventResponse.model_validate({
                ...
                if event.metadata_json else None,
            }))
